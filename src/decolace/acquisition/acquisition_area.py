...
class AcquisitionAreaSingle:
    ...
    def initialize_from_navigator(self, item=None):
        ...
        if (
            self.state["view_stage_to_specimen"] is None
            or self.state["view_specimen_to_camera"] is None
        ):
        ...
        if (
            self.state["record_speciment_to_camera"] is None
            or self.state["record_IS_to_camera"] is None
        ):
            serialem.GoToLowDoseArea("R")
            self.state["record_speciment_to_camera"] = np.array(
                serialem.SpecimenToCameraMatrix(0)
            )
            self.state["record_IS_to_camera"] = np.array(serialem.ISToCameraMatrix(0))
    ...
    def acquire(
        self, established_lock=False, initial_defocus=None, initial_beamshift=None
    ):
        ...
        for index in track(range(len(self.state["acquisition_positions"]))):
            ...
            if counts > self.state["count_threshold_for_beamshift"]:
                ...
                self.state["beamshift_calibration"]["measurements"].append(
                    {
                        "position_on_camera_before_centering": measured_beam_position,
                        "beam_shift_before_centering": beam_shift_before_centering,
                        "beam_shift_after_centering": beam_shift_after_centering,
                        "counts": counts,
            ...
            if len(ctf_results) < 6:
                ...
                print(report)
                continue
            report += f"✓ CTF: {ctf_results[0]:.2f}um DF "
            report += f"{ctf_results[4]:.2f} CC {ctf_results[5]:.2f}A Res"
            if (
...
class acquisition_area:
    def __init__(
        ...
        if load_from_file:
            ...
            # Directory key: tilt angle, value array (bool,n) indicating of position acquired
            self.state["positions_acquired"] = {}
            ...
            # Directory, key: tilt angle, value: dictionary, keys("list of positions","filename")
    ...
    def designate_calibration_positions(self, num=10):
        ...
        kmeans = KMeans(n_clusters=num, random_state=0).fit(
            self.state["acquisition_positions"]
        )
        self.state["calibration_positions"] = []
        ...
        for cluster in kmeans.cluster_centers_:
            closest_point_index = min(
                range(len(self.state["acquisition_positions"])),
                key=lambda i: abs(
                    np.linalg.norm(self.state["acquisition_positions"][i] - cluster)
                ),
            )
            self.state["calibration_positions"].append(closest_point_index)
        self.state["calibration_positions"] = np.array(
            self.state["calibration_positions"]
        )

    def move_to_position(self):
        serialem.RealignToOtherItem(
            int(self.state["navigator_index"]), 0, 0, 0.05, 4, 1
        )
    ...
    def extrapolate_defocus_calibration(self, tilt, plot_diagnostics=False):
        existing_calibrations = list(
            [
                tilt
                for tilt in self.state["defocus_calibrations"].keys()
                if "fit" in self.state["defocus_calibrations"][tilt]
                and self.state["defocus_calibrations"][tilt]["fit"][3] > 0.5
            ]
        )
        if (
            tilt in self.state["defocus_calibrations"]
            and "fit" in self.state["defocus_calibrations"][tilt]
            and self.state["defocus_calibrations"][tilt]["fit"][3] > 0.5
        ):
            print("Experimental fit exists, extrapolation unnecessary")
            return self.state["defocus_calibrations"][tilt]["fit"]
        if (
            len(
                [
                    tilts
                    for tilts in self.state["defocus_calibrations"]
                    if "fit" in self.state["defocus_calibrations"][tilts]
                    and self.state["defocus_calibrations"][tilts]["fit"][3] > 0.5
                ]
            )
            < 9
        ):

            closest_calibration_tilt = existing_calibrations[
                min(
                    range(len(existing_calibrations)),
                    key=lambda i: abs(existing_calibrations[i] - tilt),
                )
            ]
            print(
                f"Calibrating, starting from calibration at tilt {closest_calibration_tilt}"
            )
            closest_calibration = self.state["defocus_calibrations"][
                closest_calibration_tilt
            ]
            return closest_calibration

        closest_calibration_tilts = np.array(
            sorted(
                [
                    existing_calibrations[i]
                    for i in sorted(
                        range(len(existing_calibrations)),
                        key=lambda i: abs(existing_calibrations[i] - tilt),
                    )[:9]
        ...
        x_coeffs = np.array(
            [
                float(self.state["defocus_calibrations"][tilts]["fit"][0])
                for tilts in closest_calibration_tilts
            ]
        )
        y_coeffs = np.array(
            [
                float(self.state["defocus_calibrations"][tilts]["fit"][1])
                for tilts in closest_calibration_tilts
            ]
        )
        height_coeffs = np.array(
            [
                float(self.state["defocus_calibrations"][tilts]["fit"][2])
                for tilts in closest_calibration_tilts
            ]
        )
    ...
    def acquire_single_tilt_slow(self, tilt):
        ...
        for index in range(len(self.state["acquisition_positions"])):
            if self.state["positions_acquired"][tilt][index]:
                continue
            print(
                f"Acquiring image {index+1} out of {len(self.state['acquisition_positions'])}"
            )
            serialem.ImageShiftByMicrons(
                self.state["acquisition_positions"][index][0],
                self.state["acquisition_positions"][index][1],
            ...
            if self.state["ctf_tool"] == "ctffind":
                ctf_results = serialem.CtfFind("A", -0.1, -12)
                self.state["defocus_calibrations"][tilt]["measurements"].append(
                    {
                        "defocus": ctf_results[0],
                        "score": ctf_results[4],
                        "fit_resolution": ctf_results[5],
                ...
                if (
                    ctf_results[4] > self.state["ctf_quality_threshold"]
                    and counts > self.state["count_threshold_for_beamshift"]
                    and ctf_results[5] < self.state["ctf_res_threshold"]
                ):
                    offset = self.state["desired_defocus"] - ctf_results[0]
                    if offset > 0.5:
                        offset = 0.5
                    ...
                    print(
                        f"Unreliable defocus {ctf_results[4]} {ctf_results[5]}, defocusing by {self.state['ctf_step']}"
                    )
                    serialem.ChangeFocus(self.state["ctf_step"])

            if self.state["ctf_tool"] == "ctfplotter":
                ctf_results = serialem.Ctfplotter(
                    "A", -0.1, -12, 1, 0, 0, 126, 2.0, 0.168, 0.3
                )
                self.state["defocus_calibrations"][tilt]["measurements"].append(
                    {
                        "defocus": ctf_results[0],
                ...
                if ctf_results[4] > self.state["ctf_quality_threshold_ctfplotter"]:
                    ...
                    print(
                        f"Unreliable defocus {ctf_results[4]}, defocusing by {self.state['ctf_step']}"
                    )
                    serialem.ChangeFocus(self.state["ctf_step"])
    ...
    def perform_defocus_calibration(self, tilt, use_existing_tilt=False):
        ...
        for index in self.state["calibration_positions"]:
            ...
            if use_existing_tilt:
                set_defocus = (
                    closest_calibration["fit"][0]
                    * self.state["acquisition_positions"][index][0]
                    + closest_calibration["fit"][1]
                    * self.state["acquisition_positions"][index][1]
    ...
    def plot_calibrations(self):
        calibrations = [
            tilt
            for tilt in self.state["defocus_calibrations"]
            if "fit" in self.state["defocus_calibrations"][tilt]
        ]
        calibrations = sorted(calibrations)
        x_tilt = [
            float(self.state["defocus_calibrations"][tilt]["fit"][0])
            for tilt in calibrations
        ]
        y_tilt = [
            float(self.state["defocus_calibrations"][tilt]["fit"][1])
            for tilt in calibrations
        ]
        offset = [
            float(self.state["defocus_calibrations"][tilt]["fit"][2])
            for tilt in calibrations
        ]
        f, axs = plt.subplots(1, 3, figsize=(15, 5))
    ...
    def analyze_defocus_calibration(
        self, tilt, minimum_score=0.08, min_measurement=4, extrapolate_distance=10
    ):
        ...
        if len(xs) < min_measurement:
            print(
                f"At tilt {tilt} not sufficisent measurements: {len(xs)}. Using extrapolation"
            )
            self.state["defocus_calibrations"][tilt][
                "fit"
            ] = self.extrapolate_defocus_calibration(tilt)["fit"]
            return ()
        ...
        print(f"solution: {fit[0]:f} x + {fit[1]:f} y + {fit[2]:f} = z")
        # Fourth value indicates whether fit is experimental or extrapolated
        self.state["defocus_calibrations"][tilt]["fit"] = [
            float(fit[0]),
            float(fit[1]),
            float(fit[2]),
            1.0,
    ...
    def acquire_data(self, tilt, num_pos=40, only_write=False):
        serialem.TiltTo(tilt)
        set_defocus = (
            self.state["defocus_calibrations"][tilt]["fit"][2]
            + self.state["desired_defocus"]
        )
        serialem.SetDefocus(set_defocus)
        ...
        while sum(np.invert(self.state["positions_acquired"][tilt])) > 0:
            ...
            to_acquire = np.argwhere(self.state["positions_acquired"][tilt] is False)
            ...
            for index in acquire_now:
                ...
                defocus_offset = (
                    self.state["defocus_calibrations"][tilt]["fit"][0] * x
                    + self.state["defocus_calibrations"][tilt]["fit"][1] * y
                )
                tilt_series_commands.append(
                    [
                        self.state["exposure_time"],
                        self.state["exposure_time"] + 0.1,
                        defocus_offset,
                        x,
                        y,
                    ]
            ...
            if not only_write:
                serialem.ManageDewarsAndPumps(1)
                serialem.CallFunction(
                    "40::RunFISE",
                    command_file,
                    (self.state["exposure_time"] + 0.15) * len(acquire_now) + 4.0,
                )
                for index in acquire_now:
                    self.state["positions_acquired"][tilt][index] = True
                self.state["fast_acquisitions"][tilt].append(
                    {
                        "positions": acquire_now,
                        "filename": serialem.ReportLastFrameFile(),
                    }
                )
            else:
                break
