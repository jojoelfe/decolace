...
import starfile
import numpy as np
from decolace_processing import find_tile_pairs, calculate_shifts, calculate_refined_image_shifts, calculate_refined_intensity
console = Console()
...
def main(
    ...
    estimated_distance_threshold = np.median(tile_data['tile_x_size'] * tile_data['tile_pixel_size'])
    for iteration in range(iterations):
        ...
        console.log(f"In {len(tile_data)} tiles {len(tile_pairs)} tile pairs were found using distance threshold of {estimated_distance_threshold:.2f} A.")
        # Calculate shifts of tile pairs
        shifts = calculate_shifts(tile_pairs, num_procs)
        console.log(f"Shifts were adjusted by an average of {np.mean(shifts['add_shift']):.2f} pixels.")
