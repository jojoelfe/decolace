...
def new_project(name: str, directory: Path):
    ...
    project_info = pd.DataFrame.from_dict({0: [value for value in project_info.values()]},orient='index', columns=[key for key, _, _ in project_info_schema])
    ...
    aa_info = pd.DataFrame(columns=['name', 
                                    ...
                                    'cisTEM project', 
                                    'initial_tile_star', ])
    aa_info.to_sql('acquisition_areas', sqlite_connection, if_exists='fail', index=False)
    project_info.to_sql('project_info', sqlite_connection, if_exists='fail', index=False,dtype={key: sqlalchemy.types.JSON for key, _, default in project_info_schema if type(default) == dict})
        ...
        for file in directory.iterdir():
            if file.suffix == '.decolace':
                if filename is not None:
                    raise ValueError('Multiple project files found in directory. Please specify which one to load.')
                filename = file
