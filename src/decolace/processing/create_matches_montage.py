from decolace_processing import adjust_metadata_for_matches, create_montage, read_matches_data
import typer
...
def main(
    ...
    matches_montage_metadata = adjust_metadata_for_matches(
        montage_data, match_data
    )
    #console.log(f"Wrote montage metadata to {output_path_metadata}.")
    create_montage(matches_montage_metadata, output_path_montage)
    console.log(f"Wrote {matches_montage_metadata['montage']['montage_x_size'].values[0]}x{matches_montage_metadata['montage']['montage_y_size'].values[0]} montage to {output_path_montage}.")
